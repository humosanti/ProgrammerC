...
int main(void)
{
	/* The size array stores hat size from 6 1/2 to 7 7/8 */
	...
	/* Convert to whole eighths of a inch */
	your_head=(int)(8.0*cranium);

	/* Search for a hat size */
	/* A fit is when yur_head is greater that one headsize element */
	/* and less than or equal to the net. The size the the second */
	/* headsize value. */
	for(i=1;i<12;i++)
	{
	...
	else
	{
		...
		{
			printf("\nYou are the proverbial pinhead. "
					"No hat for you I'm afraid.\n");
		}
		/* It must be too large */
		else
		{
			printf("\nYou, in technical parlance, are a "
					"fathead. No hat for you, I'm"
					" afraid.\n");
		}
	}
